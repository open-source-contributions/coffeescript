...
class CoffeeScriptTest extends TestCase
{
    ...
    public function testGetResultFromRaw()
    {
        ...
        $javascript = trim($stylus->getResult());
        $expected = trim(str_replace("\r", '', file_get_contents(__DIR__ . '/test.js')));

        $this->assertSame($expected, $javascript, 'CoffeeScript should be rendered anyway.');
    }
    ...
    public function testEchoResult()
    {
        ...
        ob_start();
        echo $coffee;
        $javascript = trim(ob_get_contents());
        ob_end_clean();
        $expected = trim(str_replace("\r", '', file_get_contents(__DIR__ . '/test.js')));
    ...
    public function testNonBareGetResult()
    {
        ...
        $javascript = preg_replace('/[\s_]/', '', $coffee->getResult());
        $expected = preg_replace('/[\s_]/', '', '(function(){' . file_get_contents(__DIR__ . '/test.js') . '}).call(this);');

        $this->assertSame($expected, $javascript, 'CoffeeScript should be rendered in a function with node.');
    }
    ...
    public function testNonBareGetResultWithoutNode()
    {
        ...
        $this->assertSame($expected, $javascript, 'CoffeeScript should be rendered in a function without node.');
    }
    ...
    public function testWrite()
    {
        ...
        $coffee = new CoffeeScript(__DIR__ . '/test.coffee');
        $coffee->write($file);
        $javascript = trim(str_replace("\r", '', file_get_contents($file)));
        unlink($file);
        $expected = trim(str_replace("\r", '', file_get_contents(__DIR__ . '/test.js')));
    ...
    public function testGetResultWithoutNode()
    {
        ...
        $javascript = trim(preg_replace('/[\r_]/', '', $coffee->getResult()));
    ...
    public function testWriteWithoutNode()
    {
        ...
        $file = sys_get_temp_dir() . '/test.js';
        $coffee->write($file);
        $javascript = trim(preg_replace('/[\r_]/', '', file_get_contents($file)));
        unlink($file);
